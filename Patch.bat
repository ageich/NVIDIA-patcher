...
if not %ErrorLevel% == 0 (
	certutil -f -addstore Root EVRootCA.crt >nul
		if not !ErrorLevel! == 0 (
			echo Failed to install timestamp server root certificate^^! Install EVRootCA.crt manually into Trusted Root Certification Authorities.
			pause
			exit /b 1
		)
)
